# -*- coding: utf-8 -*-
"""
Created on Fri Sep 15 14:28:15 2017
...
#地址
def genAddress():
    area = ['安徽','山东','北京','天津','上海','浙江','江苏','河南','河北','山西','陕西','宁夏','江西','福建','广东','广西','台湾','香港','澳门',
            '四川','湖北','湖南','青海','新疆','内蒙古','西藏','辽宁','吉林','黑龙江','甘肃','云南','贵州','重庆']
    a = random.sample(area,1)
    return ''.join(a)
#生日
def genRandomDay():
...
def genDataBase(fileName,dataCount):
    ...
    first_names = ['的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '这', '个', '上', '们', '来', '到', '时', '大', '地', '为',
                   ...
                   '夏', '困', '刘', '鞋', '健', '模', '败', '伴', '守', '挥', '鲜', '财', '孤', '枪', '禁', '恐', '伙', '杰',
    ...
    while i<=100 :
        ...
        address = genAddress() 
        cursor.execute(
        "insert into tbl_user(user_address,user_birthday,user_name,user_sex) values(%s,%s,%s,%s)",(address,birthday,name,sex))
        conn.commit()
        i += 1
...
if __name__ == "__main__":
    random.seed()
    start = time.time()
    genDataBase('db_test.txt',dataCount)
    end = time.time()
